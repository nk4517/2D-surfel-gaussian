...
//			float v = (hu.z * hv.x - hu.x * hv.z) / Denom;
			...
			const float du_dhu1 = -u * hv.y / s.z;
			const float du_dhu2 = -v * hv.y / s.z;
			const float du_dhu3 = -hv.y / s.z;

			const float du_dhv1 = u * hu.y / s.z;
			const float du_dhv2 = v * hu.y / s.z;
			const float du_dhv3 = hu.y / s.z;

			const float dv_dhu1 = u * hv.x / s.z;
			const float dv_dhu2 = v * hv.x / s.z;
			const float dv_dhu3 = hv.x / s.z;

			const float dv_dhv1 = -u * hu.x / s.z;
			const float dv_dhv2 = -v * hu.x / s.z;
			const float dv_dhv3 = -hu.x / s.z;
